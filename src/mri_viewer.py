...
class MRIViewer(QMainWindow):
    ...
    def load_label_config(self, filepath=None):
        ...
        try:
            ...
            self.statusBar().showMessage(
                f"Loaded label config from: {self.label_config_path}"
            )
            return True
        except FileNotFoundError:
            self.statusBar().showMessage(
                f"Label config file not found. Using default empty map."
            )
            self.label_map = {}
            return False
    ...
    def save_label_config(self):
        ...
        try:
            ...
            self.statusBar().showMessage(
                f"Saved label config to: {self.label_config_path}"
            )
        except Exception as e:
            QMessageBox.critical(
                self, "Config Error", f"Failed to save label config: {e}"
            )
    ...
    def calculate_label_volumes(self):
        ...
        if self.mask_data is None:
            QMessageBox.warning(
                self, "Reporting Error", "No segmentation mask data loaded."
            )
            return {}
        ...
        try:
            ...
            QMessageBox.critical(
                self, "Volume Error", f"Failed to read voxel dimensions: {e}"
            )
            return {}
    ...
    def export_volume_report(self):
        ...
        try:
            ...
            QMessageBox.critical(
                self, "Export Error", f"Failed to initialize export worker: {e}"
            )
            return
        ...
        try:
            if (
                not getattr(self, "_export_worker", None)
                or not getattr(self, "_export_worker", None).isRunning()
            ):
                self.btn_cancel_export.setEnabled(False)
        except Exception:
            pass
        ...
        try:
            worker.progress.connect(
                lambda p, msg: self.statusBar().showMessage(f"Export: {p}% - {msg}")
            )
        except Exception:
            pass
        ...
        try:
            ...
            try:
                ...
                pass
            QMessageBox.critical(
                self, "Export Error", f"Failed to start export worker: {e}"
            )
            return
    ...
    def _on_cancel_export_clicked(self):
        ...
        try:
            ...
            QMessageBox.warning(
                self, "Cancel Error", f"Failed to request cancellation: {e}"
            )
    ...
    def build_ui(self):
        ...
        # Main layout uses a splitter instead of a simple horizontal box
        ...
        self.splitter.setStretchFactor(1, 4) # Vis grid
    ...
    def closeEvent(self, event):
        ...
        if worker is not None and worker.isRunning():
            ...
            try:
                if (
                    hasattr(self, "btn_cancel_export")
                    and self.btn_cancel_export.isEnabled()
                ):
            ...
            if worker.isRunning():
                QMessageBox.warning(
                    self,
                    "Close Warning",
                    "Export is still running. Close will proceed and terminate the worker.",
                )
        super().closeEvent(event)

    def build_left_panel(self):

        # Create the outer scroll area
        scroll = QScrollArea()
        scroll.setWidgetResizable(True)
        scroll.setFrameShape(QFrame.NoFrame)
        scroll.setHorizontalScrollBarPolicy(Qt.ScrollBarAlwaysOff)
        scroll.setVerticalScrollBarPolicy(Qt.ScrollBarAsNeeded)
        scroll.setStyleSheet("QScrollBar:vertical { width: 8px; }") # Slight override if needed
        ...
        self.btn_to_int.clicked.connect(
            self.convert_to_integer_labels
        )  # NEW CONNECTION

        # Set the content widget into the scroll area
        scroll.setWidget(panel)
        scroll.setMinimumWidth(320)  # Ensure it doesn't get too narrow
        scroll.setMaximumWidth(450)  # Ensure it doesn't get too wide
    ...
    def apply_wl_preset(self, index):
        ...
        self.statusBar().showMessage(
            f"Applied Preset: {txt} (W: {target_window:.1f}, L: {target_level:.1f})"
        )
    ...
    def convert_to_integer_labels(self):
        ...
        try:
            ...
            if min_val < 0:
                data -= min_val
                self.statusBar().showMessage(
                    "Warning: Negative values found and shifted to be non-negative before casting."
                )
            ...
            self.statusBar().showMessage(
                f"Conversion complete. Data type: {self.mri_data.dtype}, Min: {np.min(self.mri_data)}, Max: {np.max(self.mri_data)}"
            )
            ...
            QMessageBox.critical(
                self,
                "Conversion Error",
                f"Failed to convert to integer labels: {str(e)}",
            )
    ...
    def update_vtk_data(self):
        ...
        vtk_array = numpy_support.numpy_to_vtk(
            num_array=flat_data, deep=True, array_type=vtk_type
        )
    ...
    def apply_n4_bias_field_correction(self, data):
        ...
        if not SIMPLEITK_AVAILABLE:
            QMessageBox.critical(
                self, "Error", "SimpleITK is required for N4 Bias Field Correction."
            )
            return data
        ...
        try:
            ...
            if new_max > new_min:
                new_data = ((new_data - new_min) / (new_max - new_min)) * (
                    orig_max - orig_min
                ) + orig_min

            self.statusBar().showMessage(
                "N4 Bias Field Correction applied (5 iterations)."
            )
            return new_data
    ...
    def apply_histogram_op(self):
        ...
        if not SKIMAGE_AVAILABLE and "N4" not in self.combo_hist.currentText():
            QMessageBox.warning(
                self, "Error", "Scikit-image/Scipy is required for most operations."
            )
            return

        txt = self.combo_hist.currentText()
        if "Select" in txt or "---" in txt:
            return
        ...
        try:
            ...
            if "N4 Bias Field Correction" in txt:
                if not SIMPLEITK_AVAILABLE:
                    QMessageBox.critical(
                        self,
                        "Error",
                        "SimpleITK is required for N4 Bias Field Correction.",
                    )
            ...
            # 5. Sigmoid
                ...
                mean = np.mean(data)
                gain = 10 / (max_v - min_v)
                self.mri_data = (max_v - min_v) * (
                    1 / (1 + np.exp(-gain * (data - mean)))
                ) + min_v
            ...
            # 7. Unsharp Masking
                ...
                self.mri_data = (
                    filters.unsharp_mask(data, radius=param, amount=1.0) * max_v
                )
            ...
            # 8. Total Variation Denoising
                ...
                weight = 0.1 * param
                self.mri_data = (
                    restoration.denoise_tv_chambolle(data, weight=weight) * max_v
                )
    ...
    def apply_threshold_op(self):
        ...
        try:
            ...
            # --- AUTOMATED (N Classes) ---
                ...
                self.mri_data = indices.astype(
                    np.float64
                )  # Keep as float temporarily for VTK/next ops

                # Display thresholds found
                thresh_str = ", ".join([f"{t:.2f}" for t in thresholds])
                self.statusBar().showMessage(
                    f"Multi-Otsu calculated {n_classes} classes with thresholds: {thresh_str}"
                )
    ...
    def export_modified_mri(self):
        ...
        if not NIBABEL_AVAILABLE:
            QMessageBox.critical(
                self, "Error", "NiBabel is required to export NIfTI files."
            )
            return
        ...
        if self.mri_affine is None:
            QMessageBox.critical(
                self,
                "Error",
                "Original MRI affine matrix is missing. Cannot preserve spatial information.",
            )
            return
        ...
        try:
            ...
            new_img = nib.Nifti1Image(
                self.mri_data.astype(export_dtype), self.mri_affine
            )
            ...
            self.statusBar().showMessage(
                f"Successfully exported modified MRI to: {filename}"
            )
            QMessageBox.information(
                self, "Export Success", f"Modified MRI data saved as:\n{filename}"
            )

        except Exception as e:
            self.statusBar().showMessage("Export failed.")
            QMessageBox.critical(
                self, "Export Error", f"Failed to save NIfTI file: {str(e)}"
            )
            traceback.print_exc()
    ...
    def toggle_annotation_mode(self, checked):
        ...
        if checked:
            self.statusBar().showMessage(
                "Annotation Mode ON: Click on the 3D view to place a point."
            )
    ...
    def _setup_3d_picker(self):
        ...
        def on_right_click(obj, event):
            ...
            if world_pos and self.mri_data is not None:
                ...
                if (
                    ...
                    self.statusBar().showMessage(
                        "Annotation point is outside the volume boundaries."
                    )
        ...
        interactor_style.AddObserver(
            vtk.vtkCommand.RightButtonPressEvent, on_right_click
        )
    ...
    def _prompt_and_add_annotation(self, image_idx):
        ...
        if ok and text:
            ...
            self.annotations.append(
                {"position": image_idx, "text": text, "actor": label_actor}
            )
    ...
    def _update_annotations_on_2d_slices(self):
        ...
        for view_name in views_2d:
            ...
            props_to_remove = [
                p for p in renderer.GetViewProps() if isinstance(p, vtk.vtkActor)
            ]
            ...
            for anno in self.annotations:
                ...
                if view_name == "axial" and abs(z - current_slice_index) < tolerance:
                    ...
                    view_name == "sagittal" and abs(x - current_slice_index) < tolerance
                ):
                    is_visible = True
                elif (
                    view_name == "coronal" and abs(y - current_slice_index) < tolerance
                ):
                    is_visible = True
    ...
    def export_screenshot(self):
        if self.mri_data is None:
            QMessageBox.warning(
                self, "Warning", "Please load an MRI file before exporting."
            )
            return

        if (
            self.stacked_layout.currentIndex() == 1
            and self.current_fullscreen_view_name
        ):
        ...
        if target_view_name not in self.vtk_widgets:
            QMessageBox.critical(
                self, "Error", "Selected view is not available for export."
            )
            return
        ...
        if not render_window:
            self.statusBar().showMessage(
                "Error: Could not find a render window to export."
            )
            return
        ...
        try:
            ...
            self.statusBar().showMessage(
                f"Successfully exported screenshot to: {filename}"
            )
            QMessageBox.information(
                self, "Export Success", f"Screenshot saved as:\n{filename}"
            )
    ...
    def build_vis_grid(self):
        ...
        for view_name, (row, col) in self.view_grid_positions.items():
            ...
            fullscreen_btn.clicked.connect(
                lambda checked, v=view_name: self.toggle_fullscreen(v)
            )
    ...
    def load_mri(self):
        if not NIBABEL_AVAILABLE:
            QMessageBox.warning(
                self, "Warning", "NiBabel is not installed. Cannot load MRI files."
            )
            return
        ...
        try:
            ...
            self.mri_affine = (
                img.affine
            )  # Store the affine matrix from the image object
    ...
    def load_mask(self):
        if not NIBABEL_AVAILABLE:
            QMessageBox.warning(
                self, "Warning", "NiBabel is not installed. Cannot load mask files."
            )
            return
        ...
        try:
            ...
            self.unique_mask_values = self.unique_mask_values[
                self.unique_mask_values > 0
            ]
            ...
            vtk_arr = numpy_support.numpy_to_vtk(
                num_array=flat, deep=True, array_type=vtk.VTK_UNSIGNED_SHORT
            )
            ...
            self.statusBar().showMessage(
                f"Loaded mask with {num_labels} labels: {depth}x{height}x{width}"
            )
            QMessageBox.information(
                self, "Success", f"Successfully loaded mask with {num_labels} labels"
            )
    ...
    def setup_mask_visualization(self):
        ...
        max_label = (
            max(self.unique_mask_values) if len(self.unique_mask_values) > 0 else 1
        )
    ...
    def setup_3d_view(self):
        ...
        def StartInteraction(obj, event):
            ...
            self.volume_mapper.SetInteractiveUpdateRate(
                5.0
            )  # Allow dropping frames to keep up
    ...
    def update_axial_slice(self, value):
        ...
        if self.mask_data is not None and self.show_mask_check.isChecked():
            ...
            mask_actor.GetProperty().SetOpacity(
                self.mask_opacity_slider.value() / 100.0
            )
    ...
    def toggle_fullscreen(self, view_name):
        ...
        if container:
            ...
            self.exit_fullscreen_btn = QPushButton(
                f"Exit Fullscreen: {view_name.capitalize()} (Esc)"
            )
    ...
    def exit_fullscreen_mode(self):
        if (
            ...
            or self.current_fullscreen_view_name is None
        ):
            return
    ...
    def _get_representative_slice_index(self):
        ...
        if self.mri_data is None:
            return {"axial": 0, "coronal": 0, "sagittal": 0}
        D, H, W = self.mri_data.shape  # Z, Y, X
        return {
            "axial": D // 2,
            "coronal": H // 2,
